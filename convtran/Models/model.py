...
class ConvTran(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Parameters Initialization -----------------------------------------------
        self.d_model, seq_len = config.d_model, config.seq_len
        emb_size = config.emb_size
        num_heads = config.num_heads
        dim_ff = config.dim_ff
        self.Fix_pos_encode = config.fix_pos_encode
        ...
        # Embedding Layer -----------------------------------------------------------
        self.embed_layer = nn.Sequential(nn.Conv2d(1, emb_size*config.channel_expand, kernel_size=config.kernel_sizes, padding='same'),
                                         nn.BatchNorm2d(emb_size*config.channel_expand),
                                         nn.GELU())

        self.embed_layer2 = nn.Sequential(nn.Conv2d(emb_size*config.channel_expand, emb_size, kernel_size=[self.d_model, 1], padding='valid'),
        ...
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.flatten = nn.Flatten()
        self.out = nn.Linear(emb_size, self.n_classes * self.d_pred)
        # if self.pred_multivariate:
        #     self.out = nn.Linear(emb_size, self.n_classes * self.d_pred)
        # else:
        #     self.out = nn.Linear(emb_size, self.n_classes)
    ...
    def forward(self, x):
        ...
        logits = out.reshape((batch_size, self.d_pred, self.n_classes))
